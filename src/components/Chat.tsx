...
export const Chat: React.FC<Props> = ({ messages, agents }) => {
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      const orMessages: OpenRouterMessage[] = [...(chatMessages ?? []), userMsg].map(
        (m) => ({
          role: m.role,
          content: m.content,
        })
      ...
      setChatMessages((prev) => [
        ...(prev ?? []),
        {
          id: crypto.randomUUID(),
          role: "assistant",
          content: `Error: ${e?.message || "failed to chat"}`,
          agent: selectedAgent.name,
        },
      ]);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black text-neutral-200 grid grid-cols-[10%_50%_30%_10%]">
      ...
      <div className="flex flex-col gap-2 p-6">
        <button
          className="rounded-xl bg-emerald-500/20 px-4 py-2 text-sm font-medium text-emerald-300 hover:bg-emerald-500/30 mb-8"
